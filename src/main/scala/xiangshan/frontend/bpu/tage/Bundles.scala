...
class ProviderMeta(implicit p: Parameters) extends TageBundle {
  val useProvider: Bool            = Bool()
  val tableIdx:    UInt            = UInt(TableIdxWidth.W)
  val wayIdx:      UInt            = UInt(MaxNumWays.W)
  val takenCtr:    SaturateCounter = new SaturateCounter(TakenCtrWidth)
  val usefulCtr:   SaturateCounter = new SaturateCounter(UsefulCtrWidth)
}

class TageMeta(implicit p: Parameters) extends TageBundle {
  val providerMetaVec: Vec[ProviderMeta] = Vec(NumBtbResultEntries, new ProviderMeta)
}
